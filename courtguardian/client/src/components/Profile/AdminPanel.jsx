...
export default function AdminPanel({ user }) {
  ...
  const fetchCourtSuggestions = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5001/api/discovery/admin/suggestions?status=pending', {
        headers: {
  ...
  const reviewSuggestion = async (suggestionId, status, adminNotes = '') => {
    try {
      const token = getToken();
      const response = await fetch(`http://localhost:5001/api/discovery/admin/suggestions/${suggestionId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const fetchVerificationStats = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/verifications/stats');
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchPendingVerifications = async () => {
    ...
    try {
      const token = getToken();
      const response = await fetch('http://localhost:5001/api/verifications/admin/pending', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const reviewVerification = async (verificationId, action, adminNotes = '') => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/verifications/admin/${verificationId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
